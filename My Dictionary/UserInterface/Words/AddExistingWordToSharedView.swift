...
struct AddExistingWordToSharedView: View {
    ...
    private func addWordToSelectedDictionary() {
        guard let dictionaryId = selectedDictionaryId,
              let wordData = Word(from: word) else {
            return
        }
        
        DictionaryService.shared.addWordToSharedDictionary(dictionaryId: dictionaryId, word: wordData) { result in
            DispatchQueue.main.async {
                ...
                case .failure(let error):
                    ...
                    print("Error adding word to shared dictionary: \(error)")
                }
            }
        }
    }
