//
//  MusicDiscoveringContentView.swift
//  My Dictionary (macOS)
//
...
struct MusicDiscoveringContentView: View {
    ...
    @StateObject private var viewModel = MusicDiscoveringMacViewModel()
    ...
    var body: some View {
        ScrollView {
            if isAppleMusicAuthorized {
                ...
                .searchable(text: $viewModel.searchText, prompt: Loc.Words.search)
            } else {
            ...
            CustomSectionView(header: Loc.MusicDiscovering.View.Search.resultsTitle) {
                LazyVGrid(
                    ...
                    ForEach(Array(viewModel.searchResults.enumerated()), id: \.element.id) { index, song in
                        SongSuggestionCard(song: song, size: .init(width: width, height: width)) {
                            selectSong(
                                song,
                                context: "search",
                                position: index
...
private extension MusicDiscoveringContentView {
    ...
    func selectSong(
        ...
        if context == "history",
           ...
           let stored = SongLessonSessionService.shared.getSession(by: uuid),
           stored.isComplete,
           let storedSession = stored.toMusicDiscoveringSession(),
           let storedSong = stored.song {
            SideBarManager.shared.discoverDetail = .music(.lessonResults(session: storedSession, song: storedSong))
            return
        }
        ...
        SideBarManager.shared.discoverDetail = .music(.songInfo(song: song))
    }
...
private struct SongCardSkeleton: View {
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
