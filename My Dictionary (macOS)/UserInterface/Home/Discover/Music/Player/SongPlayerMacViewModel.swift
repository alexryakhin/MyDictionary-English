...
@MainActor
final class SongPlayerMacViewModel: BaseViewModel {
    ...
    init(config: MusicPlayerConfig) {
        self.song = config.song
        self.lyrics = config.lyrics
        super.init()
    ...
    private func generateLesson() async {
        ...
        guard hasLyrics, canRequestAI else {
            logError("Unable to generate lesson")
        ...
        do {
            ...
            await MainActor.run {
                logError("Unable to generate lesson with error: \(error)")
...
extension SongPlayerMacViewModel {
