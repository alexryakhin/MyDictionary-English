...
extension SongLesson {
    final class FillInBlankQuizViewModel: ObservableObject {
        ...
        var currentItem: FillInBlankItem? {
        ...
        var currentSelectionIsCorrect: Bool? {
            ...
            return item.options[selection] == item.blankWord
        }
        ...
        func selectOption(at optionIndex: Int) {
            ...
            let isCorrect = item.options[optionIndex] == item.blankWord
        ...
        func finishQuiz() {
            let submissions = items.enumerated().compactMap { index, item -> QuizSubmission? in
                ...
                let isCorrect = item.options.indices.contains(answerIndex) && item.options[answerIndex] == item.blankWord
